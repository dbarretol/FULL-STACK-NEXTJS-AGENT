...
- **Herramientas**: Verifica que el agente llame a `list_directory` o `write_file` correctamente (Gemini 2.5 tiene excelente soporte para Tool Use).
...
**Causa:** No se instalaron las dependencias de Google.
**Solución:** Ejecuta `uv pip install .[gemini]` o `pip install google-generativeai`.

### Error: API Key inválida
**Síntoma:** Errores 401 o "Unauthorized".
**Solución:** Verifica que `GOOGLE_API_KEY` esté correctamente cargada en `os.environ`. En Windows PowerShell usa `$env:GOOGLE_API_KEY="tu_llave"`.

### Error: `ModelThrottledException` (Rate Limiting)
**Síntoma:** El modelo responde con errores de cuota superada.
**Solución:**
- Si usas el plan gratuito de AI Studio, reduce la frecuencia de las peticiones.
- Aumenta el `timeout` en la configuración si estás procesando tareas muy pesadas.
...
- **Multimodalidad**: Puedes pasar imágenes o documentos (PDF) al agente simplemente añadiendo la ruta del archivo en el contenido del mensaje (Strands se encarga de la codificación).
